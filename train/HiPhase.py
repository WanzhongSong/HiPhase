...
class Encoder(nn.Module):
    def __init__(self, num_classes):
        ...
        self.layers.append(DownsamplerBlock(16,32))

        for x in range(0, 5):
           self.layers.append(non_bottleneck_1d(32, 0.03, 1))
...
class UpsamplerBlock (nn.Module):#Unsampling
    def __init__(self, ninput, noutput):
        super().__init__()
        self.conv = nn.ConvTranspose2d(ninput, noutput, 8, stride=8, padding=0, output_padding=0, bias=True)
        self.bn = nn.BatchNorm2d(noutput, eps=1e-3)
...
class Decoder (nn.Module):
    def __init__(self, num_classes):
        ...
        self.output_conv = nn.Conv2d(16, num_classes, 1, stride=1, padding=0,  bias=True)
